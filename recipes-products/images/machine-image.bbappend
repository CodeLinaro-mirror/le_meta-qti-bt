...
def get_bt_inc_file(d):
    product     = d.getVar('PRODUCT', True)
    basemachine = d.getVar('BASEMACHINE', True)
    if product != 'base' or '':
        inc_file_name = basemachine + "-" + product + "-synergy-image.inc"
    else:
        inc_file_name = basemachine + "-synergy-image.inc"
    ...
    if os.path.exists(img_inc_file_path):
        ...
        img_inc_file = basemachine + "-synergy-image.inc"
    return img_inc_file

include ${BASEMACHINE}/${@get_bt_inc_file(d)}
